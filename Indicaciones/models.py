...
class Indicacion(models.Model):
    ...
    emisor = models.ForeignKey(
        User, related_name='Indicacion_emisor', on_delete=models.CASCADE)
    categoria = models.ForeignKey(
        Categoria, on_delete=models.SET_NULL, null=True, blank=True)
    procedencia = models.ForeignKey(
        Unid_Org, on_delete=models.SET_NULL, null=True, blank=True)
    responsable = models.ForeignKey(
        User, related_name='Indicacion_responsable', on_delete=models.CASCADE)
    proceso = models.CharField(max_length=50, choices=PROCESOS)
    participantes = models.ManyToManyField(
        User, related_name='Indicacion_participantes', blank=True)
    con_copia = models.ManyToManyField(
        User, related_name='Indicacion_con_copia', blank=True)
